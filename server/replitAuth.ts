...
export async function setupAuth(app: Express) {
  ...
  const verify: VerifyFunction = async (
    ...
    const userType = req?.session?.pendingUserType || 'citizen';
    await upsertUser(tokens.claims(), userType);
    
    // Clear the pending userType from session
  ...
  app.get("/api/login", (req, res, next) => {
    // Store userType in session if provided
    const userType = req.query.userType as string;
    if (userType && (userType === 'citizen' || userType === 'authority')) {
